...
gulp.task 'serve', ->
  ...
  gulp.watch("#{@path.app}/index.html", ['build', reload])
  gulp.watch("#{@path.app}/views/*.html", ['build', reload])
  gulp.watch("#{@path.app}/**/*.coffee", ['build', 'test', reload])
  gulp.watch("#{@path.app}/**/*.scss", ['build', reload])
